<template>
  <div class="hello">
    <div class="container">
    <div class="desc">
      <img class="desc-img" src="../assets/icon.png" />
      <div class="desc-text">
        <div id="desc-text-title" class="desc-text-title">{{fullTitle}}</div>
        <div id="desc-text-category" class="desc-text-category">{{fullCategory}}</div>
      </div>
    </div>
    <img class="main-img" src="../assets/main-img.png" />
    <div id="main-text" class="main-text">
      <p v-for="(text, index) in fullTextList" v-bind:key="index">{{text}}</p>
    </div>
    <div class="hint">>>> 无需下载，点击继续阅读接下来的章节</div>
    <div class="finger-con">
  ...
  methods: {
    getFullText() {
      this.mockFetchData().then((data) => {
        ...
        this.fullTextList = this.splitFullText(data.fullText)
        this.fullTitle = data.fullTitle
        this.fullCategory = data.fullCategory
      })
    },
    mockFetchData() {
